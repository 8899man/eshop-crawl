http://misc.360buyimg.com/product/js/2012/promotion.js
var Promotions = {
  init: function(sku) {
        ...
        if (infoList[i].promoType == 1) {
          ...
          if ( infoList[i].discount > 0 && xiangou ) {
            if(infoList[i].minNum<=1){
              ...
              promotionsItems.push( '<em class="hl_red_bg">限购</em><em class="hl_red">每件可享受优惠价￥' + infoList[i].price + '，且' + limText + '</em>');
            }
          }
          //京豆优惠购
          if(jBean && infoList[i].price > 0) {
            ...
            promotionsItems.push( '<em class="hl_red_bg">京豆优惠购</em><em class="hl_red">使用' + infoList[i].needJBeanNum + '京豆可享受优惠价' + infoList[i].price + '元'+jBeanCondition+'</em>');
          }
          ...
          if (fullRefundType == FULL_REFUND) {
            if (fullLadderList != null && fullLadderList.length > 0) {
              $.each(fullLadderList, function (z, fullLadderValue) {
                ...
                if(fNeedMoney > 0 && fRewardMoney > 0&&fAddMoney<=0){ //满减赠
                  var tipsHtml = z == 0 ? '<em class="hl_red_bg">满送</em>' : '';
